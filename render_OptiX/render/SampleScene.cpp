...
void SampleScene::createContext()
{ 
...
const char* const SampleScene::ptxpath( const std::string& target, const std::string& base )
{
  ...
  path = std::string(sutil::samplesPTXDir()) + "/" + target + "_generated_" + base + ".ptx";
  return path.c_str();
}
...
void SampleScene::resize(unsigned int width, unsigned int height)
{
  ...
  SUTIL_CATCH(m_context->get())

  // Let the user resize any other buffers
  doResize( width, height );
}

bool SampleScene::keyPressed(unsigned char key, int x, int y)
{
  switch(key)
  {
  case 's':
  {
    std::string png_name = outfile + ".png";
    Buffer& outbuf = getOutputBuffer();
    if(m_use_vbo_buffer)
    {
      RTsize width, height;
      outbuf->getSize(width, height);
      glBindBuffer(GL_PIXEL_UNPACK_BUFFER, outbuf->getGLBOId());
      void* img = glMapBuffer(GL_PIXEL_UNPACK_BUFFER, GL_READ_ONLY);
      displayFileFromDataPNG("out.png", width, height, outbuf->getFormat(), img);
      glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER);
      glBindBuffer(GL_PIXEL_UNPACK_BUFFER, 0);
    }
    else
      displayFilePNG(png_name.c_str(), outbuf->get());
  }
  return true;
  }
  return false;
