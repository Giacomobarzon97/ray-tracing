...
vector<unsigned char> bufferToImage(RTformat format, GLsizei width, GLsizei height, void* data)
{
...
enum ImageFormat { PPM = 0, PNG };

RTresult
displayFileFromData(const char* filename, RTsize width, RTsize height, RTformat buffer_format, void* buffer_data, ImageFormat img_format)
{
  int w = static_cast<int>(width);
  int h = static_cast<int>(height);
  vector<unsigned char> pix = bufferToImage(buffer_format, w, h, buffer_data);
  switch(img_format)
  {
  case PPM: SavePPM(&pix[0], filename, w, h, 3);  break;
  case PNG: stbi_write_png(filename, w, h, 3, &pix[0], w*3); break;
  }
  return RT_SUCCESS;
}

RTresult
displayFile(const char* filename, RTbuffer buffer, ImageFormat img_format)
{
  RTresult result;
  ...
  int bw = static_cast<int>(buffer_width);
  int bh = static_cast<int>(buffer_height);
  vector<unsigned char> pix = bufferToImage(buffer_format, bw, bh, imageData);
  switch(img_format)
  {
  case PPM: SavePPM(&pix[0], filename, bw, bh, 3);  break;
  case PNG: stbi_write_png(filename, bw, bh, 3, &pix[0], bw*3); break;
  }
...
{
  return displayFile(filename, buffer, ImageFormat::PPM);
}

extern "C"
RTresult
displayFilePNG(const char* filename, RTbuffer buffer)
{
  return displayFile(filename, buffer, ImageFormat::PNG);
}

extern "C"
RTresult
displayFileFromDataPPM(const char* filename, RTsize width, RTsize height, RTformat buffer_format, void* buffer_data)
{
  return displayFileFromData(filename, width, height, buffer_format, buffer_data, ImageFormat::PPM);
}

extern "C"
RTresult
displayFileFromDataPNG(const char* filename, RTsize width, RTsize height, RTformat buffer_format, void* buffer_data)
{
  return displayFileFromData(filename, width, height, buffer_format, buffer_data, ImageFormat::PNG);
}
