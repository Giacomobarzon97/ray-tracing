...
{ 
  ...
  //
  // Hint: (a) Use the function optix::make_Ray(...) to generate a shadow ray.
  //       (b) Use the function rtTrace(...) to trace the shadow ray.

  float3 color=make_float3(0,0,0);
  for(int i=0;i<lights.size();i++){
	Ray r=make_Ray(hit_pos,lights[i].direction,0,scene_epsilon,t_hit-scene_epsilon);
