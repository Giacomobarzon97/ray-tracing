...
//-----------------------------------------------------------------------------
// 
// Functions no longer available in sutils
//-----------------------------------------------------------------------------

namespace
{
  RTresult sutilParseFloatDimensions(const char *arg, double *width, double *height)
  {
...
void GLUTDisplay::printUsage()
{
  ...
  std::cerr
    ...
    << "  right mouse           Camera Pan/Truck (when interactive)\n"
    << "  middle mouse          Camera Dolly (when interactive)\n"
    << "  middle mouse + shift  Camera FOV (when interactive)\n"
    << std::endl;
...
void GLUTDisplay::init( int& argc, char** argv )
{
  ...
  for (int i = 1; i < argc; ++i ) {
    ...
    if ( arg == "-s" || arg == "--save-frames" ) {
      ...
      try {
        int w, h;
        sutil::parseDimensions(arg_value.c_str(), w, h);
        width = w;
        height = h;
      }
      catch(Exception e) {
      ...
      try {
        int w, h;
        sutil::parseDimensions(bnd_args.c_str(), w, h);
        m_warmup_frames = w;
        m_timed_frames = h;
      }
      catch(Exception e) {
...
void GLUTDisplay::run( const std::string& title, SampleScene* scene, contDraw_E continuous_mode )
{
  ...
  */
  ...
  // Initialize timer
  m_last_frame_time = sutil::currentTime();
  m_frame_count = 0;
...
void GLUTDisplay::setCurContinuousMode(contDraw_E continuous_mode)
{
  ...
  m_start_time = sutil::currentTime();
...
void GLUTDisplay::runBenchmarkNoDisplay( )
{
  ...
  if( m_save_frames_to_file ) {
    ...
    Buffer buffer = m_scene->getOutputBuffer();
    sutil::displayBufferPPM( filename.c_str(), buffer->get() );
  }
...
void GLUTDisplay::keyPressed(unsigned char key, int x, int y)
{
  ...
  case 's':
    sutil::displayBufferPPM( "out.ppm", m_scene->getOutputBuffer()->get() );
    break;
...
void GLUTDisplay::displayFrame()
{
  ...
  if( m_save_frames_to_file ) {
    ...
    sutil::displayBufferPPM( fname, buffer->get() );
  }
...
void GLUTDisplay::quit(int return_code)
{
  try {
    if(m_scene)
    {
      ...
      {
        std::cerr << "Derived scene class failed to call SampleScene::cleanUp()";
        exit(2);
      }
    }
    exit(return_code);
